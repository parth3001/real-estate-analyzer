describe('Anna, TX Property - Aggressive Investor Validation', () => {
  ...
  it('should analyze same Anna, TX property with Aggressive Investor profile', () => {
    ...
    cy.get('body').then($body => {
      ...
      // QE DATA EXTRACTION ADDED ONLY AT END
      cy.log('🔬 QE: EXTRACTING REAL INVESTMENT DECISION ENGINE DATA FOR AI AUDIT');

      const qeExtractedData = {
        timestamp: new Date().toISOString(),
        testType: 'QE_REAL_Investment_Decision_Engine_Data',
        property: testProperty,
        verdict: verdict,
        dealQualityScore: score,
        financialMetrics: {
          capRate: capRateMatch ? capRateMatch[1] : null,
          monthlyCashFlow: cashFlowMatch ? cashFlowMatch[1] : null,
          irr: irrMatch ? irrMatch[1] : null,
          cashOnCashReturn: cocMatch ? cocMatch[1] : null,
          monthlyRent: rentMatch ? rentMatch[1] : null
        },
        rawPageContent: bodyText.substring(0, 15000) // Capture more for analysis
      };

      // Save the REAL data
      const fileName = `QE-REAL-engine-data-${Date.now()}.json`;
      cy.writeFile(`cypress/fixtures/${fileName}`, qeExtractedData);

      cy.log('💾 QE: REAL DATA EXTRACTED AND SAVED');
      cy.log(`📁 File: cypress/fixtures/${fileName}`);
      cy.log('✅ Ready for genuine AI audit validation');
    });
