describe('Anna, TX Property - Aggressive Investor Validation', () => {
  ...
  it('should analyze same Anna, TX property with Aggressive Investor profile', () => {
    ...
    cy.get('body').then($body => {
      ...
      // QE DATA EXTRACTION FOR AI AUDIT
      cy.log('🔬 QE: EXTRACTING DATA FOR AI AUDIT...');

      const extractedData = {
        timestamp: new Date().toISOString(),
        testType: 'QE_Investment_Decision_Engine_AI_Audit',
        property: testProperty,
        verdict: verdict,
        dealQualityScore: score,
        financialMetrics: {},
        rawAnalysisText: bodyText.substring(0, 10000) // First 10k chars for AI review
      };

      // Extract additional metrics
      if (capRateMatch) extractedData.financialMetrics.capRate = capRateMatch[1];
      if (cashFlowMatch) extractedData.financialMetrics.monthlyCashFlow = cashFlowMatch[1];
      if (irrMatch) extractedData.financialMetrics.irr = irrMatch[1];
      if (cocMatch) extractedData.financialMetrics.cashOnCashReturn = cocMatch[1];
      if (rentMatch) extractedData.financialMetrics.monthlyRent = rentMatch[1];

      // Save extraction data for AI audit
      const fileName = `ai-audit-data-${Date.now()}.json`;
      cy.writeFile(`cypress/fixtures/${fileName}`, extractedData);

      cy.log('💾 QE DATA SAVED FOR AI AUDIT');
      cy.log(`📁 File: cypress/fixtures/${fileName}`);
      cy.log('🚀 Ready for independent AI validation');
    });

    cy.log('🎯 Aggressive investor analysis completed with data extraction');
  });
});