...
| **Atlas dev database setup** (revised) | Create new DB user `reanalyzr_dev_user` with `readWrite` on `real-estate-analyzer-dev` database only. Update `.env` MONGODB_URI database path to `-dev`. Same cluster, different database — cost stays $0. **CRITICAL before any application code writes events.** See [PRODUCT_2.0_PROD_MIGRATION.md §2.1-2.2](PRODUCT_2.0_PROD_MIGRATION.md). | ⚠️ Founder action — 5 min Atlas dashboard work |
