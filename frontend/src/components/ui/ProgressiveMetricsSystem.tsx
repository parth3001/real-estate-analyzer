...
const ProgressiveMetricsSystem: React.FC<ProgressiveMetricsSystemProps> = ({ analysis, propertyData }) => {
  ...
  const metricsCategories: Record<string, MetricCategory> = useMemo(() => ({
    ...
    market: {
      ...
      metrics: [
        { key: 'marketRentEstimate', label: 'Market Rent Estimate', value: analysis?.marketData?.marketTrends?.medianRent || 0, format: 'currency' },
        { key: 'rentGrowthYoY', label: 'Rent Growth YoY', value: analysis?.marketData?.marketTrends?.rentGrowthRate || 0, format: 'percentage' },
        { key: 'vacancyRate', label: 'Market Vacancy Rate', value: analysis?.marketData?.marketTrends?.inventoryLevel === 'Low' ? 3 : analysis?.marketData?.marketTrends?.inventoryLevel === 'High' ? 8 : 5, format: 'percentage' },
        { key: 'daysOnMarket', label: 'Avg Days on Market', value: analysis?.marketData?.marketTrends?.daysOnMarket || 0, format: 'number' },
        { key: 'priceGrowthYoY', label: 'Price Growth YoY', value: analysis?.marketData?.marketTrends?.priceGrowthRate || 0, format: 'percentage' }
      ]
    },
